...
sealed class BerkeleyNGram(n: Int) extends EventModel[java.util.List[String]] {
  val ngramLm = n match {
    case 2 | 3 => LmReaders.readLmBinary("LM-kn" + n + "-train-100MW.binlm").asInstanceOf[ArrayEncodedProbBackoffLm[String]];
    case 4 => LmReaders.readLmBinary("LM-kn" + n + "-train-50MW.binlm").asInstanceOf[ArrayEncodedProbBackoffLm[String]];
    case 5 => LmReaders.readLmBinary("LM-kn" + n + "-train-20MW.binlm").asInstanceOf[ArrayEncodedProbBackoffLm[String]]
  }
...
class KLModel(trainSet: List[Document], models: Model*) extends Model {
  ...
  def apply(doc: Document): Double = {
    models.map(model => model(doc)).reduce((x, y) => { x-y })
  }
